...
let dir: string
beforeEach(() => {
  dir = mkdtempSync(join(tmpdir(), 'plucker-export-'))
})
...
describe('exportTracks', () => {
  it('copies each track’s current blob to dest named by its resolved tags', async () => {
    ...
    repo.insertVersion({
      ...
      parentId: null,
      blobHash: blob.hash,
      recipe: { steps: [], resolved: { tags: { artist: 'Artist', title: 'Song' } } },
      materialized: true,
      createdAt: 't'
    })
    ...
    const written = await exportTracks(
      {
        repo,
        materialize: async (id) => store.pathFor(repo.getVersion(id)!.blobHash!),
        buildName: (tags) => `${tags.artist} - ${tags.title}`
      },
      ['t1'],
      dest,
      { perPlaylistSubfolder: false }
    )
