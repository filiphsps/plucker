...
function blob(store: ReturnType<typeof createContentStore>, content: string): { hash: string; path: string; size: number } {
...
describe('materializer', () => {
  it('returns the existing blob path when a version is already materialized', async () => {
    ...
    const b = store.put(f); repo.refBlob(b, store)
    ...
    repo.insertVersion({ id: 'v1', trackId: 't1', parentId: null, blobHash: b.hash, recipe: { steps: [] }, materialized: true, createdAt: 't' })
  ...
  it('evicts the coldest replayable, non-tip, non-root version once the LRU exceeds capacity', async () => {
    ...
    const root = blob(store, 'root'); repo.refBlob(root, store)
    repo.insertVersion({ id: 'v0', trackId: 't1', parentId: null, blobHash: root.hash, recipe: { steps: [] }, materialized: true, createdAt: 't0' })
    let parent = 'v0'
    for (let i = 1; i <= 9; i++) {
      const b = blob(store, `audio-${i}`); repo.refBlob(b, store)
      repo.insertVersion({ id: `v${i}`, trackId: 't1', parentId: parent, blobHash: b.hash, recipe: { steps: [] }, materialized: true, createdAt: `t${i}` })
      parent = `v${i}`
    }
